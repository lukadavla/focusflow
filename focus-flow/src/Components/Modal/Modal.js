...
function AddTask({show, setShow, tasks, setTasks}) {
  ...
  async function handleSubmit() {
    const now = new Date();
    const deadline = new Date(taskToAdd.deadline);
    
    const isPastMonthOrYear =
      deadline.getFullYear() < now.getFullYear() ||
      (deadline.getFullYear() === now.getFullYear() && deadline.getMonth() < now.getMonth());
  ...
  return (
    ...
    <div class="input-group-prepend">
        ...
        <input min={new Date().toISOString().split('T')[0]} onChange={(e) => setTaskToAdd(task => ({...task, deadline: e.target.value}))}
...
function EditTask({show, setShow, task, setTasks, priority, difficulty, status}) {
  ...
  return (
    ...
    <div class="input-group-prepend">
        ...
        <input min={new Date().toISOString().split('T')[0]} value={taskToEdit.deadline} 
